...
import {IBaseAttUtility, TAddress, IBaseAttUtilityProvider} from '../attestationData/v0'

export const extractUtility = (
  a: IBaseAttUtility,
  valType: string,
): IBaseAttUtility | string | number | TAddress | null | IBaseAttUtilityProvider['accounts'] => {
  if (valType === 'object') {
    return a
  }
  ...
  if (a.data && ['name', 'id', 'country', 'service_types', 'website', 'accounts'].indexOf(valType as any) !== -1) {
    ...
    return data[valType as keyof IBaseAttUtilityProvider]
  }
