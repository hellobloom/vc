...
test('Verifying layer2Hash, attester address, and merkle proof', () => {
  // tslint:disable:max-line-length
  const signedAttestationHash = EthUtils.hashMessage(validMerkleTreeComponentsLegacy.dataNodes[0].signedAttestation)
  const merkleTree = EthUtils.getMerkleTreeFromComponentsLegacy(validMerkleTreeComponentsLegacy)
  const proof = formatMerkleProofForShare(merkleTree.getProof(EthU.toBuffer(signedAttestationHash)) as IProof[])
...
test('Verifying layer2Hash, attester address, and merkle proof', () => {
  const signedAttestationHash = EthUtils.hashMessage(validBatchMerkleTreeComponents.claimNodes[0].attesterSig)
  const merkleTree = EthUtils.getMerkleTreeFromComponents(validBatchMerkleTreeComponents)
  const proof = formatMerkleProofForShare(merkleTree.getProof(EthU.toBuffer(signedAttestationHash)) as IProof[])
